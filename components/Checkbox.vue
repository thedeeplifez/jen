<template>
    <div class="o-checkbox-wrapper" :class="{reverse: reverse}">
        <div class="o-checkbox" :class="{'o-switch': $props.switch, checked: shouldCheck}" @click="toggle" :style="{fontSize: size ? `${size}px` : '1rem'}"></div>
        <input type="checkbox" :checked="shouldCheck" style="display:none;" :required="required">
        <label class="form-check-label o-checkbox-label" @click="toggle"><slot></slot></label>
    </div>
</template>
